...
#undef S
  ...
  void Evaluation<T>::initialize() {
    ...
    if (Us == WHITE)
    {
        ...
        // king attacks
        attackedBy2[WHITE][ALL_PIECES] = attackedBy[WHITE][KING] = pos.attacks_from<KING>(ksq);
        attackedBy2[WHITE][KING] = 0;
        b = pe->pawn_attacks(WHITE);
        attackedBy2[WHITE][ALL_PIECES] |= attackedBy[WHITE][ALL_PIECES] & b;
        attackedBy2[WHITE][PAWN] = pawn_double_attacks_bb<WHITE>(pos.pieces(WHITE, PAWN));
        attackedBy[WHITE][PAWN] = pe->pawn_attacks(WHITE);

        // king attacks
        attackedBy2[BLACK][ALL_PIECES] = attackedBy[BLACK][KING] = pos.attacks_from<KING>(ksqB);
        attackedBy2[BLACK][KING] = 0;
        b = pe->pawn_attacks(BLACK);
        attackedBy2[BLACK][ALL_PIECES] |= attackedBy[BLACK][ALL_PIECES] & b;
        attackedBy2[BLACK][PAWN] = pawn_double_attacks_bb<BLACK>(pos.pieces(BLACK, PAWN));
  ...
  Score Evaluation<T>::pieces() {
    ...
    for (Square s = *pl; s != SQ_NONE; s = *++pl)
    {
        ...
        // NOTE Include x-ray attacks for Queens?

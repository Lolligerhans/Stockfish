...
#undef S
  ...
  Value Evaluation<T>::winnable(Score score) const {
    ...
    int u = ((mg > 0) - (mg < 0)) * Utility::clamp(complexity + 50 + (60 * (mg > 0 ? doublePasserW : doublePasserB)), -abs(mg), 0);
    int v = ((eg > 0) - (eg < 0)) * std::max(complexity + (60 * (eg > 0 ? doublePasserW : doublePasserB)), -abs(eg));

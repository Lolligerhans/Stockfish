...
namespace {
  ...
  Score Evaluation<T>::king() const {
    ...
    if (kingDanger > 100)
        score -= make_score(0,0,kingDanger * kingDanger / 4096, kingDanger / 16);
  ...
  Score Evaluation<T>::initiative(Score score) const {
    ...
    int u = ((mg > 0) - (mg < 0)) * std::max(std::min(complexity + 50, 0), -abs(mg+cg_value(score)));
    int v = ((eg > 0) - (eg < 0)) * std::max(complexity, -abs(eg + eg_value(score)));

  ...
  Score Evaluation<T>::pieces() {
    ...
    for (Square s = *pl; s != SQ_NONE; s = *++pl)
    {
        ...
        if (Pt == BISHOP || Pt == KNIGHT)
        {
            ...
            bb = (attackedBy[Us][PAWN] | shift<Down>(pos.pieces(PAWN)))
                              & ~pe->pawn_attacks_span(Them);
            Bitboard targets = pos.pieces(Them) & ~pos.pieces(PAWN);

            if (   Pt == KNIGHT
                && OutpostRanks & bb & s & ~CenterFiles // on a side outpost
                && !(b & targets)        // no relevant attacks
                && (!more_than_one(targets & (s & QueenSide ? QueenSide : KingSide))))
                score += BadOutpost;
            else if (OutpostRanks & bb & s)
                score += Outpost[Pt == BISHOP];
            else if (Pt == KNIGHT && (shift<-Down>(OutpostRanks) | shift<Down>(OutpostRanks)) & bb & b & ~pos.pieces(Us))
